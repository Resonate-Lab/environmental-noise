...
**Custom building object fields** (`customBuildings[]`): `{ id, polygon: [[lat,lng],…], baseHeightM, heightM, name, isCustom: true, userHeightOverride: true, vertexElevations: number[]|null, roofMode: 'flat'|'draped', referenceVertexIndex: number, reflectionType: string, reflectionRhoCustom: number }`. `roofMode` controls how the roof is modelled in 3D and for acoustic screening: `'flat'` = horizontal roof at `vertexElevations[referenceVertexIndex] + baseHeightM + heightM` (ASL); `'draped'` = each top vertex at `vertexElevations[i] + baseHeightM + heightM`. `referenceVertexIndex` (default 0) selects which vertex anchors the flat-roof elevation — should be the highest-ground corner. Both fields are serialised; old saves default to `'flat'`/`0`. `reflectionType` is one of `'hard_wall'`, `'windows'`, `'factory_50'`, `'open'`, `'custom'` (ISO 9613-2:1996 Table 4); defaults to `'hard_wall'` (ρ=1.0) when absent. `reflectionRhoCustom` is the user-entered ρ when `reflectionType === 'custom'`. Configured via `#cbpRoofMode` radios and `#cbpRoofRefVtx` select in the building edit panel.

**Building source object fields** (`buildingSources[]`): add `roofMode: 'flat'|'draped'` and `referenceVertexIndex: number` with the same semantics as custom buildings. `buildBsPanelHTML` exposes `input[name="bsRoofMode"]` and `#bsRoofRefVtx`. OSM buildings (`buildings[]`) also gain `roofMode` and `referenceVertexIndex` (default `'draped'`/`0`), configurable via the building popup.
